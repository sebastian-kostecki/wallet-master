...
> **Ostatnia synchronizacja:** 2026-06-06 (branch `improvement/goals-to-pockets` — rename UI goals → pockets / Kieszenie / Envelopes). Poprzednio: `feature/goals-target-model`. Architektura Variant A — **zakończona** (reguła `.cursor/rules/wallet-architecture.mdc`). PRD kanoniczny: `.docs/prd.md`.
>
...
### 19) Kieszenie (Pockets) + UX budżetu (FR-P1–P5, FR-UX1) **[plan budget-goals-ux → rename `improvement/goals-to-pockets`]**

> **Rename goals → pockets (2026-06-06):** domena `Pockets`, trasy `/pockets`, UI PL „Kieszenie”, EN „Envelopes”, `pocket_id`, telemetria `pocket_*`. Backend w `26552ea`; frontend + i18n w tym branchu.

> **Refaktor modelu (2026-06-04):** szacunki roczne/miesięczne per rok zastąpione kopertą bez roku: `target_amount`, `planning_mode`, ikona/kolor, saldo skumulowane, archiwizacja. Spec: `2026-06-04-goals-target-model-design.md`. PRD: FR-P2, §5 Pocket.

**Wave 2 UX (shipped):**

- [x] Migracje: `pockets` (legacy `goals`), `transactions.pocket_id`
- [x] CRUD kieszeni (backend + `pockets/Index.vue`, `Create.vue`, `Edit.vue`)
- [x] Kieszeń wymagana na transferze z/do konta `Savings`; opcjonalna na wydatku
- [x] Budżet miesięczny: sekcja per-kieszeń (`pocket_rows`)
- [x] Plany P&L tylko na budżecie — `categories/Index.vue` bez pól szacunków
- [x] Budżet roczny: edycja szacunków rocznych P&L inline
- [x] Nawigacja: „Kieszenie” / „Envelopes” w sidebarze (`nav.pockets`)
- [x] Migracja legacy: szacunek kategorii „Oszczędności” → kieszeń „Oszczędności ogólne`
- [x] Testy feature: Pockets, TransferPocket, TransactionPocket, MonthlyBudget (kieszenie)
- [x] Transfer bez kategorii P&L; usunięcie kategorii „Oszczędności” ze starter setu

**Target model:**

- [x] Migracja target model; `PocketPlanningMode`, `PocketBalance`, `PocketPlanningProjection`
- [x] CRUD backend: walidacja planowania; `PocketResource`
- [x] `ListPockets` z filtrem; `ReorderPockets`; archiwizacja + telemetria `pocket_archived` / `pocket_unarchived`
- [x] Trasy: `pockets` resource, `pockets.reorder`
- [x] Frontend: reorder, progress bar, `PocketBadge`, `PocketArchiveDialog`
- [x] Locale: `pockets.*`, `budget.monthly.pockets_section`

**Waluta kieszeni:**

- [x] Migracja: `pockets.currency_id` (backfill PLN)
- [x] Backend + frontend: formatowanie kwot z symbolem waluty
- [x] Testy: `PocketCurrencyTest`, `PocketCurrencyMigrationTest`, unit `PocketBalance`
