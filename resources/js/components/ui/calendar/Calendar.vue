...
const selectedParts = computed(() => isoToParts(props.modelValue));

const viewYear = ref<number>(selectedParts.value?.y ?? new Date().getUTCFullYear());
const viewMonth = ref<number>(selectedParts.value?.m ?? new Date().getUTCMonth() + 1);
...
const grid = computed(() => {
    ...
    // next month fillers up to 6 rows
    const target = 42;
    ...
    <div class="w-72">
        ...
        <div class="mt-1 grid grid-cols-7 gap-1">
            <button
                ...
                :class="
                    cn(
                        'inline-flex h-9 w-9 items-center justify-center rounded-md text-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring',
                        cell.isCurrentMonth ? 'text-foreground hover:bg-muted' : 'text-muted-foreground hover:bg-muted/70',
                        cell.iso === modelValue ? 'bg-primary text-primary-foreground hover:bg-primary' : '',
                    )
                "
